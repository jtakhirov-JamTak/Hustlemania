...
```sh
npx supabase db reset            # blank local DB + every migration in supabase/migrations (schema, functions, job)
...
|---|---|---|---|---|---|
| 2026-09-12 20:10 | `C:\Users\jtakh\backups\hustlemania\2026-09-12-f17` (`roles.sql` 370 B, `data.sql` 39 KB) | 7 / 1 / 14 / 0 (also 1 vision, 1 impediment, 1 cue, 11 situations, 5 + 6 attachments, 5 + 6 situation observations, 2 tasks; counted from the dump's COPY blocks) | identical on all thirteen counts after `db reset --version 0020` → the user's `psql` load (one transaction, 41 COPY blocks, exit 0) → `migration up` applied 0021 and 0022 on the real rows: the 11 situations renumbered 1..11 with the five impediment ones first in their old order, no duplicate rank, attachments and observation rows kept, the sprint row kept without its `why` ("A"), `cues.explanation` and `situation_kind` gone, one 20-argument `start_sprint`, `day_offered_items` still answering 2 per day | not opened (the count and rank checks were the proof; the screens are covered by the F17 e2e and the 48 captures) | Third drill, F17 release rehearsal — the dump-and-migrate order of §Release order, run before `db push`. Local stack reset to blank afterwards. |
| 2026-09-12 07:25 | `C:\Users\jtakh\backups\hustlemania\2026-09-12-f16` (`roles.sql` 370 B, `data.sql` 38 KB) | 7 / 1 / 14 / 0 (also visions 1, impediments 1; read through the service role) | identical, all six, after `db reset --version 0019` → the user's `psql` load (one transaction, 41 COPY blocks, exit 0) → `migration up` applied 0020 on the real rows: the vision kept its goal, proof, deadline 2028-01-13 and obstacle with a complete rule, `picture` null, the three dropped columns gone | not opened (the step count and gate were proven by the row query and by `scripts/rehearse-0020.mjs` earlier) | Second drill, F16 release rehearsal — the dump-and-migrate order of §Release order, run before `db push`. This also discharges the restore owed on the 2026-09-12 pre-0019 dump (the owner decided its rows had no value after the wipe; the load path itself is now proven twice). Local stack reset to blank afterwards. |
